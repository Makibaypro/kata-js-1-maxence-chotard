...
const decrireMeteo = (conditions, temperature) => {
  // Écris ton code ici
  let conseils = "";
  if(temperature < 0)        conseils = "Temps glacial";
  else if(temperature < 10)  conseils = "Temps froid";
  else if(temperature < 20)  conseils = "Temps frais";
  else if(temperature < 30)  conseils = "Temps agréable";
  else                       conseils = "Temps chaud";

  if(conditions === "pluvieux" || conditions === "orageux") conseils += " et pluvieux";
  if(conditions === "neige")                                conseils += " et enneigé";
  if(conditions === "brouillard")                           conseils += " et brumeux";
...
const recommanderTenue = (conditions, temperature) => {
  ...
  let recommandationArray = [];

  if(temperature < 0)   recommandationArray.push({ icone: "🧥", texte: "Manteau d'hiver indispensable" });
  else if(temperature < 10)  recommandationArray.push({ icone: "🧥", texte: "Prends un manteau" });
  else if(temperature < 20)  recommandationArray.push({ icone: "🧣", texte: "Une veste suffira" });
  else if(temperature >= 30) recommandationArray.push({ icone: "👕", texte: "Habits légers recommandés" });

  if(conditions === "pluvieux")                       recommandationArray.push({ icone: "☂️", texte: "N'oublie pas ton parapluie" });
  if(conditions === "orageux")                        recommandationArray.push({ icone: "⚠️", texte: "Évite les zones exposées" });
  if(conditions === "neige")                          recommandationArray.push({ icone: "👢", texte: "Préfère des bottes imperméables" });
  if(conditions === "ensoleille" && temperature > 20) recommandationArray.push({ icone: "🕶️", texte: "Pense à la crème solaire" });

  return recommandationArray;
...
1 - Car la condition ne peux s'appliquer qu'une seul fois.

2 - Plusieurs conditions se remplissent : 
    - ensoleille et 28C donc emoji SoleilVisage
    - On est sous 30 donc temps agréable
    - et ensoleille + au dessus de 20 donc on conseil de la creme solaire avec l'icone des lunettes.
